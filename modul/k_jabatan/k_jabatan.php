...
switch($_GET[act]){
	...
	$tanggal= date('Y-m-d');
	$tampil=mysql_query("select * from pegawai a inner join bagian b on a.id_bag=b.id_bag inner join jabatan c on a.id_jab=c.id_jab where a.status_peg=8 Limit 10");
	echo "<h2 class='head'>DATA KENAIKAN JABATAN</h2>
...
echo "  
	...
	$tampil=mysql_query("select * from pegawai a inner join bagian b on a.id_bag=b.id_bag inner join jabatan c on a.id_jab=c.id_jab where a.status_peg=8 and a.nip like '%$cari%' OR a.nama like '%$cari%' Limit 10");
	echo "<h2 class='head'>CARI DATA KENAIKAN JABATAN</h2>
	<input type=button value='Reset' onclick=\"window.location.href='?module=kjb';\">
  ...
  $no++;
	...
	case "ubah":
	$tanggal2= date('Y-m-d');
	$tampil2=mysql_query("select * from pegawai,bagian,jabatan where pegawai.id_jab=jabatan.id_jab and pegawai.id_bag=bagian.id_bag and nip='$_GET[id]' limit 1");
	echo "<h2 class='head'>DATA KENAIKAN JABATAN</h2>
	<table class='tabel'>
	<thead>
  <tr>
...
echo "  
	...
	<input type=button value='cancel' onclick=\"window.location.href='?module=kjb';\">";
	} else {
