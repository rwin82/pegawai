...
switch($_GET[act]){
	...
	if(!isset($_POST[input_cari])){
		$tampil=mysql_query("select * from pegawai a
		inner join bagian b on a.id_bag=b.id_bag
		inner join jabatan c on a.id_jab=c.id_jab
		left join cabang d on a.id_cab=d.id_cab
		where a.status_peg='8'
		order by nip ASC limit 100");


echo "<h2 class='head'>DATA KARYAWAN</h2>
	<table width='100%' border='0'><tr><td>
	<input type=button value='Tambah Data' onclick=\"window.location.href='?module=pegawai&act=input';\">
	</td><td align='right'>
  ...
  echo "<tr>
    <td>$no</td>
    <td>$dt[nip]</td>
    <td><span><a href='?module=pegawai&act=detail&id=$dt[nip]'>$dt[nama]</a></span></td>
	...
	if(isset($_POST[input_cari])){
		...
		inner join jabatan c on a.id_jab=c.id_jab
		left join cabang d on a.id_cab=d.id_cab
		where a.status_peg='8' and nip like '%$cari%' OR nama like '%$cari%'
		order by nip ASC limit 100" );
	
	echo "<h2 class='head'>CARI DATA KARYAWAN</h2>
	<input type=button value='Reset' onclick=\"window.location.href='?module=pegawai';\">
	<table width=100% class='tabel'>
	<thead>
  <tr>
    <td>No</td>
...
echo " 
	...
	case "input":
	echo "<h2 class='head'>Entry Data Karyawan</h2>
	...
	<tr>
	<td>Divisi</td><td>:</td><td><select name='bagian'>
